# Hi, I'm Hassan Elsayed 👋

🎓 Cybersecurity Student — Faculty of Artificial Intelligence, Delta University  
🔍 Bug Bounty Hunter | Web Application Security Researcher

---

## 🧠 About Me

I'm a passionate cybersecurity student focused on web application security and ethical hacking.  
I actively hunt for vulnerabilities on bug bounty platforms and apply what I learn in real-world targets.

---

## 🛠️ Skills & Tools

**Vulnerability Testing**
- SQLi, XSS, SSRF, Command Injection, Broken Authentication
- CSRF, CORS Misconfiguration, Path Traversal, Access Control Vulnerabilities
- Information Disclosure, Business Logic Vulnerabilities, Race Conditions, API Testing

**Reconnaissance & OSINT**
- Subfinder, HTTPX, GAU, Katana, ParamSpider, Whois, Dig, Hunter.how

**Exploitation & Scanning**
- Burp Suite, Dalfox, XSStrike, SQLMap, FFUF, Nuclei, Mantra

**Fingerprinting**
- Wappalyzer

---

## 🎯 Bug Bounty

🐛 **HackerOne** — hassan0x30

**Focus areas:** Web apps · API security · Access control flaws · Injection vulnerabilities

---

## 📚 Currently

- 🔭 Working on bug bounty programs (HackerOne)
- 🌱 Deepening skills in advanced web exploitation & WAF bypass
- 🎓 Studying Ethical Hacking & Digital Forensics

---

## 📫 Contact

[![LinkedIn](https://img.shields.io/badge/LinkedIn-Hassan%20Elsayed-blue?style=for-the-badge&logo=linkedin)](https://www.linkedin.com/in/hassan-elsayed-5a7098275)

[![Medium](https://img.shields.io/badge/Medium-Writeups-black?style=for-the-badge&logo=medium)](https://medium.com/@hassanelsayed30)
